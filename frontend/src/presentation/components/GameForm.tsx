...
export function GameForm() {
    ...
    async function createNewGame(width: number, height: number, mineCount: number) {
        try {
            const gameRoom = await client.create("minesweeper", { width, height, mineCount });
    ...
    return (
        <div className="flex flex-col items-center w-full sm:w-96 h-full bg-gray-700 shadow-lg rounded-md">
            ...
            <InputField label="Mines" id="mines" value={minePercentage} rangeMin={10} rangeMax={40} suffix={"%"} onChange={setMinePercentage}/>
